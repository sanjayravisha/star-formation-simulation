...
tEnds = 8  # end time for simulation
dt = 0.05  # timestep
M = 2  # star mass
R = 0.75  # star radius
...
for tEnd in tEnds:
  ...
  out = cv2.VideoWriter('simulation-tEnd=' + str(tEnd) + '&h=' + str(h) +'.mp4', cv2.VideoWriter_fourcc(*'MP4V'), 30, size)
  ...
  print("Average rho for tEnd = ", tEnd, ": ", rho_avg)
  print("Average P for tEnd = ", tEnd, ": ", p_avg)
